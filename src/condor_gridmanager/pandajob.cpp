...
void PandaJobReconfig()
{
	int tmp_int;

	tmp_int = param_integer( "GRIDMANAGER_GAHP_CALL_TIMEOUT", 5 * 60 );
	PandaJob::setGahpCallTimeout( tmp_int );

	// Tell all the resource objects to deal with their new config values
	PandaResource *next_resource;

	PandaResource::ResourcesByName.startIterations();

	while ( PandaResource::ResourcesByName.iterate( next_resource ) != 0 ) {
		next_resource->Reconfig();
	}
}

bool PandaJobAdMatch( const ClassAd *job_ad ) {
	int universe;
	std::string resource;
...
BaseJob *PandaJobCreate( ClassAd *jobad )
{
	return (BaseJob *)new PandaJob( jobad );
}

int PandaJob::submitInterval = 300;	// default value
int PandaJob::gahpCallTimeout = 300;	// default value
int PandaJob::maxConnectFailures = 3;	// default value

PandaJob::PandaJob( ClassAd *classad )
	: BaseJob( classad )
{
	char buff[4096];
	...
	myResource = PandaResource::FindOrCreateResource( resourceManagerString,
														  jobProxy );
...
void PandaJob::Reconfig()
{
	BaseJob::Reconfig();
	gahp->setTimeout( gahpCallTimeout );
}

void PandaJob::doEvaluateState()
{
	int old_gm_state;
...
BaseResource *PandaJob::GetResource()
{
	return (BaseResource *)myResource;
}

void PandaJob::SetRemoteJobId( const char *job_id )
{
	free( remoteJobId );
	if ( job_id ) {
...
std::string *PandaJob::buildSubmitRSL()
{
...
StringList *PandaJob::buildStageInList()
{
...
StringList *PandaJob::buildStageOutList( bool old_stdout )
{
	StringList *stage_list = NULL;
	std::string buf;
	bool transfer = true;
...
StringList *PandaJob::buildStageOutLocalList( StringList *stage_list,
												  bool old_stdout )
{
...
void PandaJob::NotifyNewRemoteStatus( const char *status )
{
...
void PandaJob::GetRemoteStdoutNames( std::string &std_out, std::string &std_err, bool use_old_names )
{
	if ( use_old_names ) {
